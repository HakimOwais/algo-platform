"""HAR-RV (Corsi 2009) and EGARCH(1,1) volatility models — pure Python."""
...
from app.quant._numeric import EPS, clean, mean
...
class EGARCHResult:
    omega: float
    alpha: float
    gamma: float    # leverage: γ < 0 → down-moves amplify vol
    beta: float
    next_variance: float
    forecast_vol_annual: float
...
def _ols_4(
    X: list[tuple[float, float, float]], y: list[float]
) -> tuple[float, float, float, float]:
    """OLS for design matrix [1, x1, x2, x3] via normal equations with partial-pivot GE."""
    n = len(y)
    if n < 5:
...
def estimate_har_rv(
    ...
    for t in range(22, len(rv)):
        rv_d = rv[t - 1]
        rv_w = mean(rv[max(0, t - 5) : t])
        rv_m = mean(rv[max(0, t - 22) : t])
    ...
    rv_w = mean(rv[-5:])
    rv_m = mean(rv[-22:])
    forecast_var = max(alpha + b_d * rv_d + b_w * rv_w + b_m * rv_m, EPS)
    ...
    return HARRVResult(
        ...
        forecast_vol_annual=math.sqrt(forecast_var) * math.sqrt(steps_per_year),
    )
...
def estimate_egarch(
    ...
    γ < 0 captures the leverage effect.
    ...
    if len(arr) < 10:
        sample_var = sum(r * r for r in arr) / max(len(arr), 1) if arr else EPS
        return EGARCHResult(
            omega=math.log(max(sample_var, EPS)),
    ...
    sample_var = max(sum(r * r for r in arr) / len(arr), EPS)
        ...
        for g in gamma_grid:
            for b in beta_grid:
                ...
                for i in range(1, len(arr)):
                    z = arr[i - 1] / math.sqrt(max(math.exp(log_h), EPS))
                    log_h = omega + a * (abs(z) - E_abs_z) + g * z + b * log_h
                    if not math.isfinite(log_h):
                        valid = False
                        break
                    log_r2 = math.log(max(arr[i] ** 2, EPS))
    ...
    log_h = log_sv
    for r in arr:
        z = r / math.sqrt(max(math.exp(log_h), EPS))
        log_h = omega + alpha * (abs(z) - E_abs_z) + gamma * z + beta * log_h

    next_var = max(math.exp(log_h), EPS)
    return EGARCHResult(
        omega=omega,
        alpha=alpha,
        gamma=gamma,
